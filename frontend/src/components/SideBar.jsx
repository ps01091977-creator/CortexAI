...
import { LogOut, Menu, MessageSquare, PanelLeftIcon, PanelRight, PenBoxIcon, PenSquare, Plus, User, X, Trash2 } from "lucide-react"
import { useState } from 'react'
import { useEffect } from 'react'
import { getConversations } from '../features/getConversations'
import { useDispatch, useSelector } from 'react-redux'
import { addConversation, setConversations, setSelectedConversation, removeConversation, resetConversations } from '../redux/conversationSlice'
                    ...
                    {conversations?.map((conv, i) => {
                        ...
                        return (
                            <div
                                key={conv?._id || i}
                                onClick={() => dispatch(setSelectedConversation(conv))}
                                className={`group flex items-center justify-between cursor-pointer mb-0.5 px-3 py-2 rounded-[10px] border transition-all duration-150
                ${isActive ? "bg-indigo-500/10 border-indigo-500/[0.18]"
                    ...
                    ${isActive ? "bg-indigo-500/15 text-indigo-400" : "bg-white/[0.05] text-slate-500"}`}>
                                ...
                                <button
                                    className='opacity-0 group-hover:opacity-100 flex items-center justify-center w-6 h-6 rounded-md hover:bg-red-500/20 text-slate-500 hover:text-red-400 transition-all duration-150 border-none bg-transparent cursor-pointer'
                                    onClick={async (e) => {
                                        e.stopPropagation();
